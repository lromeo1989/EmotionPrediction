...
for out=1:10
    ...
    for in=1:5
        ...
        for idBC=1:numel(BC)
            ...
            MISVMmodelin=MI_Pattern_SVM(BagTrainPosNormVal,BagTrainNegNormVal,BC(idBC));
    ...
    for ginout=1:size(BagTEnorm,1)
        ...
        [SVM_test(ginout,1),loc_wind(out,ginout)]=max(score_MISVMtest(:,2));
        score_mitot{out,ginout}=score_MISVMtest(:,2);
        [~,score_MISVMtest2]=predict(MISVMmodelout2,BagTEnorm{ginout,1});
        score_mitotPost{out,ginout}=score_MISVMtest2(:,2);
        score1tot(ginout,1)=max(score_MISVMtest2(:,2));
    end
    coeffMod{out}=MISVMmodelout.Beta;
...
save('Results_miSVM_3MIL_Valence')
