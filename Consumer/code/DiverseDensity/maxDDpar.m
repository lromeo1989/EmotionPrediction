function [Concepts,maxConcept,Iterations]=maxDDpar(PBags,NBags,Dim,Scales,SPoints,Epochs,Tol)
...
%  Begin Diverse Density maximizing
  ...
  %     disp(strcat('Dfpmin completed in epochs: ',num2str(iter),'......'));
   ...
   [maxConcept{3}, ord_max]= max(cell2mat(E));
   Cmat=cell2mat(C);
   Dmat=cell2mat(D);
   maxConcept{1}=Cmat(ord_max,:);
   maxConcept{2}=Dmat(ord_max,:);
