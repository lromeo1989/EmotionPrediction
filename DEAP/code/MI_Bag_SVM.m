...
%     Y1=[lab_neg_instances; lab_pos_Bag_avg];
    ...
    while(select~=0)
        count=count+1;
%         VD = hyperparameters('fitcsvm',X1,Y1);
%         VD=VD(1,1);
%         VD.Range=[0.1 10];
        mod1=fitcsvm(X1,Y1,'KernelFunction','Linear','BoxConstraint',BC);%'OptimizeHyperparameters',VD,'HyperparameterOptimizationOptions',struct('Optimizer','randomsearch','MaxObjectiveEvaluations',20,'AcquisitionFunctionName','expected-improvement-plus','ShowPlots',false,'Verbose',0));      
        for np=1:nposBag
