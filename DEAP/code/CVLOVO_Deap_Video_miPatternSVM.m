...
MISVMmodel=MI_Pattern_SVM(BagTrainValPosNorm,BagTrainValNegNorm,BC(idBC));
...
for ji=1:length(threshold)    
    ...
    %accth=[accth (sum(ypred2==lab_only_val_c))/length(lab_only_val_c)];
...
%% mi-SVM
MISVMmodel_test=MI_Pattern_SVM(BagTrainPosNorm,BagTrainNegNorm,BC(R));

coeffMod{out,in1}=MISVMmodel_test.Beta;

[~,score_MISVMtest]=predict(MISVMmodel_test,TE1);
scoretot_mi{out,in1}=score_MISVMtest(:,2);
[SVM_test,loc_wind(out,in1)]=max(score_MISVMtest(:,2));
...
scoretot_miPost{out,in1}=score_MISVMtest2(:,2);
